	...
	var admin = {
		...
		req: function(url, data, success, method) {
			...
			$.ajax({
				...
				async: true,
				success: function(data) {
					//console.log('用户进来了111'+JSON.stringify(data))
					success(data);
				},
				error: function(xhr) {
					console.log(xhr.status + ' - ' + xhr.statusText);
		...
		aj: function(url, info, success, method,cont) {
			var token = config.getToken();
			var tokeninfo = '';
			...
			if('put' == method.toLowerCase() || 'delete' == method.toLowerCase() ||
				'post' == method.toLowerCase() || 'get' == method.toLowerCase()) {
